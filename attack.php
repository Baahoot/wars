...
if(($level > 1) & ($level/2) > ($userp['level'])) {
$message = die('<div id="FightResults"><span class="Fail">Error: '.$attacked_name.' Must Be At least Level '.number_format($level/2).' Before You Can Attack Them!</span></div>');
}
if($stamina < 1) {
...
if(($level > 1) & ($level) < ($userp['level']/2)) {
$check_retal = mysql_query("SELECT * FROM retaliation WHERE owner_id=".$user_id." AND user_id=".$id."");
if(mysql_num_rows($check_retal) == 0) {
$insert_retal = mysql_query("INSERT INTO retaliation (owner_id,user_id,attacks) VALUES ('$user_id','$id','1')");	
}
else {
$insert_retal = mysql_query("UPDATE retaliation SET attacks=(attacks+1) WHERE owner_id=".$user_id." AND user_id=".$id."");	
}
}
// Check Attacked Cash
